---
description: 지식과 문서·청크를 생성·조회·수정·검색하는 API입니다.
---
...
<figure><img src="../../.gitbook/assets/image (120).png" alt="" width="374"><figcaption></figcaption></figure>
...
지식 단위 API 키는 해당 지식에 대한 API만 호출할 수 있으며, 워크스페이스 레벨 API(지식 생성, 목록 조회, 삭제)는 호출할 수 없습니다.
...
```bash
curl --location --request POST 'http://localhost:5001/ext/v1/datasets/c2f9e6f1-d9be-41bf-a123-9db93f5a2983/docs/file' \
--header 'Authorization: Bearer {api_key}' \
...
| `embedding_model_provider` | 임베딩 모델 제공자                   |
| `retrieval_model_dict`     | 검색 설정 (검색 방식, 리랭킹, top\_k 등) |
| `app_count`                | 연결된 애플리케이션 수                 |
| `document_count`           | 포함된 문서 수                     |
| `word_count`               | 전체 단어 수                      |
| `tags`                     | 태그 목록                        |
| `document_indexing_status` | 문서 인덱싱 상태 (완료/진행중/오류 수)      |
| `embedding_available`      | 임베딩 모델 사용 가능 여부              |
