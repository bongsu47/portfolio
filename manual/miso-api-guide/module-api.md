---
description: 워크플로우를 외부에서 실행하고 결과·로그를 조회하는 API입니다.
---
...
`response_mode`에 따른 응답 형식

* `blocking` 모드일 경우: **`CompletionResponse` 객체**를 반환
...
* `streaming` 모드일 경우: **`ChunkCompletionResponse` 스트림**을 반환
